...
export type WebServer = {
    ...
    createServer: (defaultHandler?: WebHandler, options?: { errorHandler?: WebHandler, notFoundHandler?: WebHandler }) => Server;
}

type Server = {
    get: (path: string, handler: WebHandler) => Server;
    /** registers a WebSocket route with the server */
    websocket: (path: string, handler: WebSocketHandler) => Server;
